...
function makeQACards(n: number): QACard[] {
  return Array.from({ length: n }, (_, i) => ({ front: `Question ${i}`, back: `Answer ${i}` }));
}

function makeClozeCards(n: number): ClozeCard[] {
  return Array.from({ length: n }, (_, i) => ({ text: `The answer is {{c1::${i}}}.` }));
}
...
describe("AnkiDeckService", () => {
  ...
  it("splits into multiple files when cards exceed MAX_CARDS_PER_FILE", async () => {
    ...
    result.savedPaths.forEach((path) => {
  ...
  it("handles 1000+ cards without memory errors (no WASM limit)", async () => {
    ...
    result.savedPaths.filter(existsSync).forEach(unlinkSync);
  }, 60_000);

  it("returns savedPaths with .apkg extension", async () => {
    const service = new AnkiDeckService();
    const result = await service.save({
      deckName: "My Deck",
      outputPath: OUTPUT,
